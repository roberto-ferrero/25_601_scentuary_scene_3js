...
void main() {
    ...
    vec2 noisePos = vec2(position.x, position.y)/uAspectRatio;
    noisePos *= vec2(10000000.001, 10000000.001); // Modificamos la escala de la textura de ruido

    vec4 modelViewPosition = modelViewMatrix * vec4(position, 1.0);
	gl_Position = projectionMatrix * modelViewPosition;

    float uTimeSlow0  = uTime; 
    float uTimeSlow1 = uTime*1.1; // Modificamos la velocidad en la que evolucionan los gradientes
    ...
    // v_colorStrength1 = get_escalaNoise(noisePos.x, noisePos.y, 2.0, uTimeSlow1+100.)*1.;
    ...
    // v_colorStrength2 = clamp(v_colorStrength2, 0., 1.0 ); // clamp(valor, min, max)

    // v_colorStrength3 = get_escalaNoise(position.x-3000., newPositionY+400., 2.2/uResponsiveScale, uTimeSlow3+400.)*1.;
    // v_colorStrength3 = clamp(v_colorStrength3, 0., 1.0 ); // clamp(valor, min, max)
